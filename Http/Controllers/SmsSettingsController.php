...
class SmsSettingsController extends AccountBaseController
{
    ...
    {
        parent::__construct();
        $this->pageTitle = ('sms::app.smsSetting');
    ...
    public function update(StoreSmsSetting $request, $id)
    {
        ...
        if ($request->active_gateway == 'IPPanel') {
            ...
            $smsSetting->admin_from_number = $request->admin_from_number;
            $smsSetting->status = 1;
            $smsSetting->admin_status = $request->admin_status;
        }
    ...
    public function sendTestMessage(\Modules\Sms\Http\Requests\TestMessage $request)
    {
        ...
        if ($this->smsSettings->status) {
            ...
            return Reply::success('عملیات موفق');
        }
